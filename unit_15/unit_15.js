...
const f4 = () => {
    ...
    let rez = s4.has(inp);
    document.querySelector('.out-4').textContent = rez;
}
...
const f5 = () => {
    let rez = s5.size;
    document.querySelector('.out-5').textContent = rez;
}
...
let s7 = new Set();
    ...
    let arrS = Array.from(s7);
    if (arrS.length == arr.length && arr.length > 6 && arrS.length > 6) {
...
const f10 = (out_set, elem) => {
    let arr = Array.from(out_set).join(' ');
    document.querySelector(elem).textContent = arr;
}
